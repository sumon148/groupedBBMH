...
o.theta.inf.seafood <- optim(c(0,0),loglik_group_trbb ,ty=summ.seafood.data$ty,freq=summ.seafood.data$freq,b=13,
                             ...
                             G=1e4,hessian=FALSE,sensitivity=1,specificity=1,cutoff = 0)
...
temp <- loglik_group_trbb(o.theta.inf.seafood$par,ty=summ.seafood.data$ty,freq=summ.seafood.data$freq,b=13,B=8000,
                          theta=Inf,m=5,M=40,deviance=FALSE,leakage=TRUE,G=1e4,
...
o.theta.inf.seafood.sn.80 <- optim(c(0,0),loglik_group_trbb,ty=summ.seafood.data$ty,freq=summ.seafood.data$freq,b=13,
                                   theta=Inf,m=5,M=40,deviance=FALSE,control=list(reltol=1e-12,fnscale=-1),
                                   G=1e4,hessian=FALSE,sensitivity=0.80,specificity=1,cutoff = 0)
...
temp <- loglik_group_trbb(o.theta.inf.seafood.sn.80$par,ty=summ.seafood.data$ty,freq=summ.seafood.data$freq,b=13,B=8000,
                          theta=Inf,m=5,M=40,deviance=FALSE,leakage=TRUE,G=1e4,sensitivity=0.80,specificity=1,cutoff = 0)
...
for (i in 1:dim(df.pllf.sn.sp.alpha)[1]){
  ...
  tryCatch(
    ...
    {o.theta.inf.seafood.sp <- optim(c(0),loglik_group_trbb,ty=c(summ.seafood.data$ty),freq=c(summ.seafood.data$freq),b=13,m=5,
                                     theta=Inf,G=1e4,alpha=df.pllf.sn.sp.alpha$alpha[i],
    ...
    temp <- loglik_group_trbb(c(log(df.pllf.sn.sp.alpha$alpha[i]),o.theta.inf.seafood.sp$par),ty=summ.seafood.data$ty,freq=summ.seafood.data$freq,b=13,B=8000,
                              theta=Inf,m=5,M=40,leakage=TRUE,G=1e4,
                              sensitivity=df.pllf.sn.sp.alpha$sensitivity[i],
                              specificity=df.pllf.sn.sp.alpha$specificity[i],
                              cutoff=0)
...
save(df.pllf.sn.sp.alpha.prawn,file="submission/df.pllf.sn.sp.alpha.prawn.updated.rdata")
...
load("submission/df.pllf.sn.sp.alpha.prawn.updated.rdata")
...
for (i in 1:dim(df.pllf.sn.sp.mu)[1]){
  ...
  tryCatch(
    ...
    {o.theta.inf.seafood.sp <- optim(c(0),loglik_group_trbb,ty=c(summ.seafood.data$ty),freq=c(summ.seafood.data$freq),b=13,m=5,
                                     theta=Inf,G=1e4,mu=df.pllf.sn.sp.mu$mu[i], sensitivity=df.pllf.sn.sp.mu$sensitivity[i],specificity=df.pllf.sn.sp.mu$specificity[i],
    ...
    temp <- loglik_group_trbb(c(log(o.theta.inf.seafood.sp$alpha),log(o.theta.inf.seafood.sp$beta)),ty=summ.seafood.data$ty,freq=summ.seafood.data$freq,b=13,B=8000,
                              theta=Inf,m=5,M=40,leakage=TRUE,G=1e4,sensitivity=df.pllf.sn.sp.mu$sensitivity[i],specificity=df.pllf.sn.sp.mu$specificity[i])
    o.theta.inf.seafood.sp$P.L <- temp["P.L"]
    o.theta.inf.seafood.sp$E.L <- temp["E.L"]
...
save(df.pllf.sn.sp.mu.prawn,file="submission/df.pllf.sn.sp.mu.prawn.updated.rdata")
...
load("submission/df.pllf.sn.sp.alpha.prawn.updated.rdata")
load("submission/df.pllf.sn.sp.mu.prawn.updated.rdata")

leakage_estimate.sn.1 <- estimate_leakage(B=8000,b=13,M=40,m=5,
                                          alpha=df.pllf.sn.sp.alpha.prawn$alpha[df.pllf.sn.sp.alpha.prawn$sensitivity==1.0],beta=df.pllf.sn.sp.alpha.prawn$beta[df.pllf.sn.sp.alpha.prawn$sensitivity==1.0],
                                          cutoff = 0,
                                          sensitivity = 1)
leakage_estimate.sn.0.8 <- estimate_leakage(B=8000,b=13,M=40,m=5,
...
df.pllf.sn.sp.alpha.prawn$P.L[df.pllf.sn.sp.alpha.prawn$sensitivity==1.0] <- leakage_estimate.sn.1$P.L
df.pllf.sn.sp.alpha.prawn$E.L[df.pllf.sn.sp.alpha.prawn$sensitivity==1.0] <- leakage_estimate.sn.1$E.L
df.pllf.sn.sp.alpha.prawn$P.L[df.pllf.sn.sp.alpha.prawn$sensitivity==0.8] <-  leakage_estimate.sn.0.8$P.L
df.pllf.sn.sp.alpha.prawn$E.L[df.pllf.sn.sp.alpha.prawn$sensitivity==0.8] <-  leakage_estimate.sn.0.8$E.L


pllf.CI.alpha.sn.1 <- pllfCIestimate(df.pllf.sn.sp.alpha.prawn$alpha[df.pllf.sn.sp.alpha.prawn$sensitivity==1],
                                     logL=df.pllf.sn.sp.alpha.prawn$logL[df.pllf.sn.sp.alpha.prawn$sensitivity==1],level = 0.95)
...
pllf.CI.beta.sn.1 <- pllfCIestimate(df.pllf.sn.sp.mu.prawn$beta[df.pllf.sn.sp.mu.prawn$sensitivity==1],
                                    logL=df.pllf.sn.sp.mu.prawn$logL[df.pllf.sn.sp.mu.prawn$sensitivity==1],level = 0.95)
...
pllf.CI.alpha.sn.0.80 <- pllfCIestimate(df.pllf.sn.sp.alpha.prawn$alpha[df.pllf.sn.sp.alpha.prawn$sensitivity==0.80],
                                        logL=df.pllf.sn.sp.alpha.prawn$logL[df.pllf.sn.sp.alpha.prawn$sensitivity==0.80],level = 0.95)
...
pllf.CI.beta.sn.0.80 <- pllfCIestimate(df.pllf.sn.sp.mu.prawn$beta[df.pllf.sn.sp.mu.prawn$sensitivity==0.80],
                                       logL=df.pllf.sn.sp.mu.prawn$logL[df.pllf.sn.sp.mu.prawn$sensitivity==0.80],level = 0.95)
...
save(m1.prawn.a,file="submission/m1.prawn.a.Rdata")

# ---------------------------------------------------------------
# Results from Approximate model : HMC Estimator
# ---------------------------------------------------------------

load("submission/m1.prawn.a.Rdata")
...
G=3000;R=50e3;
par <- c(log(0.005), groupedBBMH::logit(7e-04)) ; initial <- par; k <- 0.0000;
MH.alpha.mu.sigma.20.Perfect.Prawn.TBB.0 <- MH_Sampler_BB(par=par,cutoff=k,initial=par,alpha=TRUE,beta=FALSE,mu=TRUE,rho=FALSE,G=G,R=R,
                                                          sigma=0.2,num_chains=4,burnin=R*0.5,thin=5,seed=c(123,456,789,135),trail=b,ty=ty2,wt=wt,group.size=Nbar, sensitivity=FALSE,specificity=FALSE,
                                                          sensitivity.range=NULL,specificity.range=NULL)
save(MH.alpha.mu.sigma.20.Perfect.Prawn.TBB.0,file="submission/MH.alpha.mu.sigma.20.Perfect.Prawn.TBB.0.Rdata")
...
MH.alpha.mu.sigma.20.unknown.sn.80.Prawn <- MH_Sampler_BB(par=par,cutoff=k,initial=par,alpha=TRUE,beta=FALSE,mu=TRUE,rho=FALSE,G=G,R=R,
                                                          sigma=c(0.2,0.2,2.0),num_chains=4,burnin=R*0.5,thin=5,seed=c(123,456,789,135),trail=b,ty=ty2,wt=wt,group.size=Nbar,sensitivity=TRUE,specificity=FALSE,
                                                          sensitivity.range=c(0.75,0.85),specificity.range=NULL)
save(MH.alpha.mu.sigma.20.unknown.sn.80.Prawn,file="submission/MH.alpha.mu.sigma.20.unknown.sn.80.Prawn.Rdata")
...
load("submission/MH.alpha.mu.sigma.20.Perfect.Prawn.TBB.0.Rdata")
...
# E.Li  22.19664 14.64824 31.54576
# P.Li   0.04107  0.02656  0.05932
...
E.L.P.L.Draws.imperfect.known.sn.k.0 <- estimate_leakage(B=8000,b=13,M=40,m=5,
                                                         ...
                                                         cutoff = 0)
MH.Estimate.imperfect.known.sn.80 <- summary_mcmc(E.L.P.L.Draws.imperfect.known.sn.k.0$alpha,
                                                  ...
                                                  varnames = c("alpha","beta","mu","E.L","P.L"))
round(MH.Estimate.imperfect.known.sn.80$summary_mcmc[,c("Mean","2.5%","97.5%")],5)
# Mean     2.5%    97.5%
...
write.csv(Results.Exact.Approx,file="submission/ML.MH.BRMS.Results.Exact.Approx.JASA.csv")
save(Results.Exact.Approx,file="submission/ML.MH.BRMS.Results.Exact.Approx.JASA.Rdata")

# Create Table 3 : Main Manuscript ----------------

load("submission/ML.MH.BRMS.Results.Exact.Approx.JASA.Rdata")
library(xtable)
# Table 2
...
param_table <- Results.Exact.Approx %>%
  ...
  dplyr::select(Scenario, Method, alpha, beta, mu, E.L, P.L) %>%

...
```
This vignette introduces the `groupedBBMH` package, which implements hierarchical Beta-Binomial models for group-testing data under imperfect test accuracy. We demonstrate both MLE and Bayesian inference using a real biosecurity dataset, with emphasis on leakage risk estimation and model-based simulations.
...
You can install the development version of `groupedBBMH` as below.
...
- $\widetilde{t}_{yi}$: Number of positive tests among sampled groups, $\widetilde{t}_{yi} = \sum_{j=1}^{b} \widetilde{Y}_{ij}$  
- $L_i$: Leakage in consignment $i$, defined as $L_i = T_{Xi} \, I(\widetilde{t}_{yi}=0)$  
...
\[
\widetilde{Y}_{ij} \mid p_i \sim \text{Bernoulli}(\widetilde{\phi}_i), 
\]

and the observed number of positive groups in the sample follows:

\[
\widetilde{t}_{yi} \mid p_i \sim \text{Binomial}(b, \widetilde{\phi}_i).
\]

where the effective probability of a positive test is: 

\[
\widetilde{\phi}_i = \Delta \phi_i + (1 - \Lambda)(1 - \phi_i) ; \quad \phi_i = 1 - (1 - p_i)^m.
\]
...
\[
\widetilde{\phi}_i = \Delta \phi_i.
\]

When $\beta \gg \alpha$, the contamination prevalence $\beta p_i$ is approximately Gamma distributed, leading to:

\[
\widetilde{\phi}_i \sim (1 - \Lambda) + \text{Beta} \left( \alpha, \frac{\beta}{m(\Delta + \Lambda - 1)} \right).
\]

Then two important special cases can be considered as:

* **Perfect testing** ($\Delta = 1$, $\Lambda = 1$): $\widetilde{\phi}_i \sim \text{Beta}(\alpha, \beta / m)$,
* **Perfect specificity** ($\Lambda = 1$): $\widetilde{\phi}_i \sim \text{Beta}(\alpha, \frac{\beta}{m \Delta})$.

In either case, $\widetilde{t}_{yi}$ approximately follows a Beta-Binomial distribution when the infection is rare:

\[
\widetilde{t}_{yi} \sim \textbf{Beta-Binomial}\ \Bigg(b, \alpha, \frac{\beta}{m \Delta}\Bigg).
\]
...
The following equations from the Das et al. (2025) form the statistical foundation of the `groupedBBMH` package:

- **Equation (1) & (2):** Two-level BB model for $X_{ij}$ 
- **Equation (3), (4) & (5):** Two-level BB model for $\widetilde{t}_{yi}$ under imperfect testing
- **Equation (6):** PMF of $\widetilde{t}_{yi}$ under imperfect testing  
- **Equation (12):** Approximate distribution $\widetilde{t}_{yi}$  under imperfect testing  
- **Equation (12):** Approximate distribution $\widetilde{t}_{yi}$  under imperfect sensitivity  
- **Equation (16):** Exact $\Pr[L_i>0]$ under perfect testing
- **Equation (17):** Exact $E(L_i)$ under perfect testing
- **Equation (18):** Approximate $\Pr[L_i>0]$ under imperfect sensitivity
- **Equation (19):** Approximate $E(L_i)$ under imperfect sensitivity
...
$\widetilde{t}_{yi}$, is summarized as 
...
```{r chunk 1, eval=TRUE, echo=TRUE, message=FALSE,warning=FALSE}
ty=c(0:13)
...
```{r chunk 2, eval=TRUE, echo=TRUE, message=FALSE,warning=FALSE}
MLE.BB.m6 <- optim(
...
```{r chunk 3, eval=TRUE, echo=TRUE, message=FALSE,warning=FALSE}
bb.m1 <- fit_GroupedBB(ty = ty, freq = freq, b = 13, m = 5,
                       sensitivity = 1, specificity = 1)
bb.m1
```

```{r chunk 4, eval=TRUE, echo=TRUE, message=FALSE,warning=FALSE}
...
```{r chunk 5, eval=TRUE, echo=TRUE, message=FALSE,warning=FALSE}
...
```{r chunk 6, eval=TRUE, echo=TRUE, message=FALSE,warning=FALSE}
...
```{r chunk 7, eval=TRUE, echo=TRUE, message=FALSE,warning=FALSE}
...
```{r chunk 8, eval=TRUE, echo=TRUE, message=FALSE,warning=FALSE}
...
```{r chunk 9, eval=TRUE, echo=TRUE, message=FALSE,warning=FALSE}
...
```{r chunk 10, eval=TRUE, echo=TRUE, message=FALSE,warning=FALSE}
...
```{r chunk 11, eval=TRUE, echo=TRUE, message=FALSE,warning=FALSE}
...
```{r chunk 12, eval=TRUE, echo=TRUE, message=FALSE,warning=FALSE}
...
For doing Posterior predictive check of the fitted model, we use the posterior distribution of $\alpha$ and $\beta$ obtained from the fitted model `MH.trbb.m1` considering $k=0.01$ and $\Delta=0.8$. The comparison of counts and their proportions from the observed outcomes are compared with simulated counts. In the below two bar-plots, the shaded areas represent the observed counts (proportions), while the error bars indicate the 95\% credible interval of simulated counts (proportions). For the plot of counts, the left y-axis is scaled from 0 to 3000 for zero value of $\widetilde{t}_{yi}$, while the right y-axis is scaled from 0 to 30 for non-zero values of $\widetilde{t}_{yi}$.
